      ...
      for (const fieldname in source) {
        for (const rulename in source[fieldname]) {
          ...
          if (rulename === 'required' && inpt[fieldname] === '') {
            ...
            if (rulename === 'min' && String(inpt[fieldname]).length < rulevalue) validationError.push(`${fieldname.replace(/_/g, ' ')} field must contain at least ${rulevalue} characters`);
            if (rulename === 'max' && String(inpt[fieldname]).length > rulevalue) validationError.push(`${fieldname.replace(/_/g, ' ')} field must contain at most ${rulevalue} characters`);
